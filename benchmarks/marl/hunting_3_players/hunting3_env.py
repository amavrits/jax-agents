...
class EnvState:
    ...
    distance: Float[Array, "n_predators"]
...
class EnvParams:
    ...
    predator_velocity: Tuple[float, float] = (.1, .1)
    predator_radius: Tuple[float, float] = (.2, .2)
    max_time: float = 1.
...
class HuntingBase(environment.Environment):
    ...
    def _distance(self, positions: POSITIONS) -> Float[Array, "1"]:
        prey_pos = jnp.take(positions, 0, axis=0)
        pred1_pos = jnp.take(positions, 1, axis=0)
        pred2_pos = jnp.take(positions, 2, axis=0)
        distance_1 = jnp.linalg.norm(prey_pos-pred1_pos)
        distance_2 = jnp.linalg.norm(prey_pos-pred2_pos)
        distance = jnp.stack((distance_1, distance_2))
        return distance

    def _rewards(self, prey_caught: Bool[Array, "n_predators"], env_params: EnvParams) -> REWARDS:
        step_rewards = jnp.asarray([+1, -1, -1]) * env_params.time_step_reward
        pred_caught_rewards = jnp.where(prey_caught, +1, +0.5)
        caught_rewards = jnp.concatenate((jnp.asarray([-1]), pred_caught_rewards)) * env_params.caught_reward
        rewards = step_rewards * (1 - jnp.any(prey_caught)) + caught_rewards * jnp.any(prey_caught)
        return rewards
    ...
    def step_env(self, key: chex.PRNGKey, state: EnvState, actions: ACTIONS, env_params: EnvParams) \
        ...
        velocities = jnp.expand_dims(jnp.concatenate((jnp.asarray([env_params.prey_velocity]), jnp.asarray(env_params.predator_velocity))), axis=-1)
        ...
        prey_caught = jnp.less_equal(next_distance, jnp.asarray(env_params.predator_radius))
        truncated = jnp.greater_equal(time, env_params.max_time)  # Truncation = time over
        terminated = jnp.logical_or(jnp.any(prey_caught), truncated)
    ...
    def render(self, time: Float[Array, "1"], positions: POSITIONS, actions: ACTIONS, values: REWARDS,
        ...
        ax.scatter(positions[2, 0], positions[2, 1], c="g", s=80, label="Predator")
        ...
        ax.arrow(positions[2, 0], positions[2, 1], dx[2], dy[2], color="g", linewidth=1)
        ...
        ax.annotate("${V}_{s}$" + "={value:.2f}".format(value=values[1]), (positions[1, 0], positions[1, 1] + 0.01),
                    color="r")
        ax.annotate("${V}_{s}$" + "={value:.2f}".format(value=values[2]), (positions[2, 0], positions[2, 1] + 0.01),
                    color="g")
        pred1_circle = plt.Circle((positions[1, 0], positions[1, 1]), env_params.predator_radius[0], color='r', alpha=0.3)
        pred2_circle = plt.Circle((positions[2, 0], positions[2, 1]), env_params.predator_radius[1], color='g', alpha=0.3)
        ax.add_patch(pred1_circle)
        ax.add_patch(pred2_circle)
...
if __name__ == "__main__":
    ...
    if discrete:
        gif_path = r"mlp/figures/random_discrete_policy.gif"
    else:
