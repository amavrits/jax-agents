...
class PGActorDiscrete(nn.Module):
    ...
    def __call__(self, x):
        ...
        activation_mask = jnp.take(x, jnp.arange(106, 118), axis=-1).astype(jnp.bool).squeeze()
        x = jnp.take(x, jnp.arange(106), axis=-1)
        ...
        logits = activation(logits)
        logits = nn.Dense(action_dim*n_actors, kernel_init=orthogonal(0.01), bias_init=constant(0.0))(logits)

        logits = logits.squeeze()
        logits = jnp.where(activation_mask, -jnp.inf, logits)

        return logits.reshape(n_actors, action_dim)
