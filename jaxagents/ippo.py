...
class IPPOAgent:
    ...
    def _init_optimizer(self, optimizer:optax.chain, optimizer_params: OptimizerParams) -> optax.chain:
        """
        ...
        optimizer_arg_names = optimizer.__code__.co_varnames  # List names of args of optimizer.
        ...
        tx = optax.chain(
            ...
            optimizer(**optimizer_params_dict)
        )

        return tx

    def _init_network(self, rng: PRNGKeyArray, network: Type[flax.linen.Module], config: IPPOConfig,
                      dummy_state: Float[Array, "state_size"]) -> Tuple[Type[flax.linen.Module], FrozenDict]:
        """
        Initialization of the actor or critic network.
        :param config: Configuration for IPPO training.
    ...
    def _create_training(self, rng: PRNGKeyArray, network: Type[flax.linen.Module], optimizer:optax.chain,
                         optimizer_params: OptimizerParams, config: IPPOConfig,
                         dummy_state: Float[Array, "state_size"]) -> TrainState:
        """
        ...
        network, params = self._init_network(rng, network, config, dummy_state)
        tx = self._init_optimizer(optimizer, optimizer_params)
    ...
    def _process_trajectory(self, update_runner: Runner, traj_batch: Transition, last_state: STATE_TYPE) -> Transition:
        ...
        traj_batch = self._add_next_values(traj_batch, last_state, update_runner.critic_trainings)
    ...
    def _actor_epoch(self, epoch_runner: Tuple[TrainState, tuple, float, int, float]) \
        ...
        actor_training, actor_loss_input, kl, epoch, kl_threshold, actor_epochs, batch_size, minibatch_size = epoch_runner
        minibatch_runner = (actor_training, actor_loss_input, 0)
        n_minibatch_updates = batch_size // minibatch_size

        actor_grad_fn = jax.grad(self._actor_loss, has_aux=True, allow_int=True)
        actor_minibatch_fn = lambda x, y: self._actor_minibatch_update(x, y, actor_grad_fn)
        minibatch_runner = lax.fori_loop(0, n_minibatch_updates, actor_minibatch_fn, minibatch_runner)
        actor_training, _, kl = minibatch_runner

        return actor_training, actor_loss_input, kl, epoch + 1, kl_threshold, actor_epochs, batch_size, minibatch_size
    ...
    def _actor_training_cond(self, epoch_runner: tuple) -> Bool[Array, "1"]:
        ...
        _, _, kl, epoch, kl_threshold, actor_epochs, batch_size, minibatch_size = epoch_runner
        return jnp.logical_and(
            jnp.less(epoch, actor_epochs),
            jnp.less_equal(kl, kl_threshold)
        )

    @partial(jax.jit, static_argnums=(0, 4, 5, 6, 7, ))
    def _actor_update(self, update_runner: Runner, actor_training: TrainState, traj_batch: Transition,
                      batch_size: int, minibatch_size: int, actor_epochs: int, i_agent: int) -> TrainState:
        """
        ...
        actor_loss_input = self._actor_loss_input(update_runner, traj_batch, batch_size, minibatch_size, i_agent)
        ...
        actor_training, _, _, _, _, _, _, _ = actor_epoch_runner
    ...
    def _critic_epoch(self, i_epoch: int, epoch_runner: Tuple[TrainState, tuple]) -> Tuple[TrainState, tuple]:
        ...
        critic_training, critic_loss_input, batch_size, minibatch_size = epoch_runner
        minibatch_runner = (critic_training, critic_loss_input)
        n_minibatch_updates = batch_size // minibatch_size
        
        critic_grad_fn = jax.grad(self._critic_loss, has_aux=False, allow_int=True)
        critic_minibatch_fn = lambda x, y: self._critic_minibatch_update(x, y, critic_grad_fn)
        minibatch_runner = lax.fori_loop(0, n_minibatch_updates, critic_minibatch_fn, minibatch_runner)
        critic_training, _ = minibatch_runner

        epoch_runner = (critic_training, critic_loss_input, batch_size, minibatch_size)

        return epoch_runner

    @partial(jax.jit, static_argnums=(0, 4, 5, 6, 7, ))
    def _critic_update(self, update_runner: Runner, critic_training: TrainState, traj_batch: Transition, batch_size: int,
                       minibatch_size: int, critic_epochs:int, i_agent: int) -> TrainState:
        """
        ...
        critic_loss_input = self._critic_loss_input(update_runner, traj_batch, batch_size, minibatch_size, i_agent)
        critic_epoch_runner = (critic_training, critic_loss_input, batch_size, minibatch_size)
        critic_epoch_runner = lax.fori_loop(0, critic_epochs, self._critic_epoch, critic_epoch_runner)
        critic_training, _, _, _ = critic_epoch_runner
    ...
    def _actor_loss_input(self, update_runner: Runner, traj_batch: Transition, batch_size: int, minibatch_size: int, i_agent: int) -> tuple:
        """
        ...
        traj_minibatch = jax.tree_map(lambda x: x.reshape(-1, minibatch_size, *x.shape[1:]), traj_minibatch)

        return (
            traj_minibatch.state,
            jnp.take(traj_minibatch.action, i_agent, axis=-1),
            jnp.take(traj_minibatch.log_prob, i_agent, axis=-1),
            jnp.take(traj_minibatch.advantage, i_agent, axis=-1),
            update_runner.hyperparams
        )

    @partial(jax.jit, static_argnums=(0, 3, 4, 5, ))
    def _critic_loss_input(self, update_runner: Runner, traj_batch: Transition, batch_size: int, minibatch_size: int, i_agent: int) -> tuple:
        """
        ...
        return (
            traj_minibatch.state,
            jnp.take(traj_minibatch.advantage, i_agent, axis=-1) + jnp.take(traj_minibatch.next_value, i_agent, axis=-1),
            update_runner.hyperparams
        )
...
class IPPO:
    ...
    def __init__(self, env: Environment, env_params: EnvParams, agents: Tuple[IPPOAgent], config: IPPOConfig, eval_during_training: bool = True):
        self.env = env
        self.agents = agents
        self.config = config
    ...
    def _create_trainings(self, rng: PRNGKeyArray, optimizer_params: Tuple[OptimizerParams, OptimizerParams])\
            -> Tuple[List[TrainState], List[TrainState]]:

        rng, rng_reset = jax.random.split(rng)
        dummy_state, _ = self.env.reset(rng_reset, self.env_params)

        # FIXME: use lax.scan ?
        actor_trainings, critic_trainings = [], []
        for (agent, optimizer) in zip(self.agents, self.config.optimizers):

            rng, rng_actor, rng_critic = jax.random.split(rng, 3)

            actor_training = agent._create_training(rng_actor, agent.actor_network, optimizer, optimizer_params[0], self.config, dummy_state)
            actor_trainings.append(actor_training)

            critic_training = agent._create_training(rng_actor, agent.critic_network, optimizer, optimizer_params[1], self.config, dummy_state)
            critic_trainings.append(critic_training)

        self.actor_trainings = actor_trainings
        self.critic_trainings = critic_trainings


    @partial(jax.jit, static_argnums=(0,))
    def _create_update_runner(self, rng: PRNGKeyArray, actor_trainings: List[TrainState],
                              critic_trainings: List[TrainState], hyperparams: HyperParameters) -> Runner:
        """
        Initializes the update runner as a Runner object. The runner contains batch_size initializations of the
        environment, which are used for sampling trajectories. The update runner has one TrainState for the actor and
        one for the critic network, so that trajectory batches are used to train the same parameters.
        :param rng: Random key for initialization.
        :param actor_trainings: The actor TrainState objects used in training.
        :param critic_trainings: The critic TrainState objects used in training.
        :param hyperparams: An instance of HyperParameters for training.
        :return: An update runner object to be used in trajectory sampling and training.
        """
        ...
        update_runner = Runner(
            actor_trainings=actor_trainings,
            critic_trainings=critic_trainings,
            env_state=env_state,
            state=state,
            rng=runner_rngs,
            hyperparams=hyperparams
    ...
    def _eval_body(self, eval_runner: tuple) -> tuple:
        ...
        env_state, state, actor_trainings, terminated, _, sum_rewards, rng = eval_runner

        actions = self.policy(actor_trainings, state)

        rng, next_state, next_env_state, reward, terminated, info = self._env_step(rng, env_state, actions)

        sum_rewards += reward

        eval_runner = (next_env_state, next_state, actor_trainings, terminated, info, sum_rewards, rng)

        return eval_runner

    def _eval_agent(self, rng: PRNGKeyArray, actor_trainings: TrainState, n_episodes: int = 1) -> Float[Array, "batch_size"]:
        """
        ...
        _, _, _, _, info, sum_rewards, _ = eval_runner
    ...
    def train(self, rng: PRNGKeyArray, hyperparams: HyperParameters) -> Tuple[Runner, Dict]:
        ...
        rng, *_rng = jax.random.split(rng, 3)
        create_trainings_rng, runner_rng = _rng

        optimizer_params = (hyperparams.actor_optimizer_params, hyperparams.critic_optimizer_params)
        self._create_trainings(create_trainings_rng, optimizer_params)

        update_runner = self._create_update_runner(runner_rng, self.actor_trainings, self.critic_trainings, hyperparams)
        ...
        # self._actor_minibatch_fn = [
        #     lambda x, y: agent._actor_minibatch_update(x, y, jax.grad(agent._actor_loss, has_aux=True, allow_int=True))
        #     for agent in self.agents
        # ]
        # self._critic_minibatch_fn = [
        #     lambda x, y: agent._critic_minibatch_fn(x, y, jax.grad(agent._critic_loss, has_aux=True, allow_int=True))
        #     for agent in self.agents
        # ]
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _pis(self, actor_trainings: TrainState, state: STATE_TYPE)-> List[PI_DIST_TYPE]:
        """
        Assess the stochastic policy via the actor network and the value via the critic network for the state.
        :param actor_trainings: The actor TrainState object used in training.
        :param critic_trainings: The critic TrainState object used in training.
        :param state: The current state of the episode step in array format.
        :return: A tuple of the stochastic policy and the value of the state.
        """
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _pis_values(self, actor_trainings: TrainState, critic_trainings: TrainState, state: STATE_TYPE)\
            -> Tuple[PI_DIST_TYPE, Float[Array, "1"]]:
        """
        Assess the stochastic policy via the actor network and the value via the critic network for the state.
        :param actor_trainings: The actor TrainState object used in training.
        ...
        return pis, jnp.asarray(values)

    @partial(jax.jit, static_argnums=(0,))
    def _select_actions(self, rng: PRNGKeyArray, pis: List[PI_DIST_TYPE]) -> Tuple[PRNGKeyArray, List[Int[Array, "1"]]]:
        """
        Select action by sampling from the stochastic policy for a state.
        :param rng: Random key for initialization.
        :param pi: The distax distribution procuded by the actor network indicating the stochastic policy for a state.
    ...
    def _rollout(self, step_runner: STEP_RUNNER_TYPE, i_step: int) -> Tuple[STEP_RUNNER_TYPE, Transition]:
        ...
        env_state, state, actor_trainings, critic_trainings, rng = step_runner

        pis, values = self._pis_values(actor_trainings, critic_trainings, state)
        ...
        step_runner = (next_env_state, next_state, actor_trainings, critic_trainings, rng)
    ...
    def _process_trajectory(self, update_runner: Runner, traj_batch: Transition, last_state: STATE_TYPE) -> Transition:
        ...
        """Remove first entry so that the next state values per step are in sync with the state rewards."""
        next_values_t = jnp.concatenate(
            [traj_batch.value.squeeze(), last_state_value],
            axis=1)[:, 1:, :]
        ...
        implementation choice is negligible.
        """
        end_advantage = jnp.zeros((self.config.batch_size, len(self.agents)))
        _, advantages = jax.lax.scan(self._trajectory_advantages, end_advantage, traj_runner, reverse=True)
    ...
    @partial(jax.jit, static_argnums=(0,))
    def policy(self, actor_trainings, state: STATE_TYPE) -> Int[Array, "1"]:
        """
        Evaluates the action of the optimal policy (argmax) according to the trained agent for the given state.
        :param state: The current state of the episode step in array format.
        :return:
    ...
    def eval(self, rng: PRNGKeyArray, actor_trainings: List[TrainState], n_evals: int = 32) -> Float[Array, "n_evals"]:
        ...
        eval_metrics = self._eval_agent(rng, actor_trainings, n_evals)
