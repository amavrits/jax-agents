...
class IPPO:
    ...
    def _create_training(self, rng: PRNGKeyArray, network: Type[flax.linen.Module], optimizer_params: OptimizerParams)\
        ...
        # return TrainState.create(apply_fn=network.apply, sample_fn=network.sample, policy_fn=network.mode, tx=tx, params=params)
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _pis(self, actor_training: TrainState, state: STATE_TYPE)-> List[PI_DIST_TYPE]:
        """
        ...
        pis = list(map(lambda x: x.apply_fn(x.params, state), actor_training))

        return pis

    @partial(jax.jit, static_argnums=(0,))
    def _pis_values(self, actor_training: TrainState, critic_training: TrainState, state: STATE_TYPE)\
        ...
        pis = list(map(lambda x: x.apply_fn(x.params, state), actor_training))
        values = jnp.asarray(list(map(lambda x: x.apply_fn(x.params, state), critic_training))).squeeze()
    ...
    @abstractmethod
    def _mode(selff, actor_training: TrainState, state: STATE_TYPE) -> Float[Array, "n_agents"]:
        raise NotImplemented
    ...
    def _process_trajectory(self, update_runner: Runner, traj_batch: Transition, last_state: STATE_TYPE) -> Transition:
        ...
        """
        n_agents = last_state_value.shape[-1]
        end_advantage = jnp.zeros((self.config.batch_size, n_agents))
    ...
    def policy(self, actor_training: TrainState, state: STATE_TYPE) -> Int[Array, "1"]:
        ...
        # pis = self._pis(actor_training, state)
        ...
        actions = self._mode(actor_training, state)
        return actions
